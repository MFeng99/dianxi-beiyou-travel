...
def has_conflict(path):
    """检测文件里有没有 git 冲突标记，有就不能上传"""
    try:
        txt = path.read_text(encoding="utf-8", errors="ignore")
    except Exception:
        return False
    return any(ln.startswith(("<<<<<<< ", ">>>>>>> ")) for ln in txt.split("\n"))
...
def main():
    ...
    # 0) 冲突防护：rebase/pull 失败会在文件里插冲突标记，绝不能上传
    bad = [f for f in FILES if (ROOT / f).exists() and has_conflict(ROOT / f)]
    if bad:
        log("\n[错误] 以下文件含有未解决的 git 冲突标记：")
        for f in bad:
            log("      - " + f)
        log("\n请先解决冲突后再上传，否则会把损坏的内容推到线上。")
        pause("按回车退出...")
        sys.exit(1)
    ...
    if code != 0:
        log("      git 连不上 github.com（国内常见），改用 API")
    else:
        log("      git 网络正常")
        ...
        if code2 == 0:
            ...
            # rebase 冲突会把仓库卡在中间状态并往文件里插标记，必须撤干净
            run("git rebase --abort")
            log("      本地与远程冲突，已撤销 rebase，改用 API 上传")

    # 3) 推送（git 通就 push，否则 API）
    ok = False
    if net_ok and not any(has_conflict(ROOT / f) for f in FILES if (ROOT / f).exists()):
        log("\n[3/4] git push ...")
